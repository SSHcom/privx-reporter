...
def render(fetch_data: Callable[[], dict[str, object]], *, top_n: int | None = None) -> None:
    ...
    st.subheader(f"Top {selected_top_n} API Users")
    st.caption("Source: Synced DB (requires Sync Server)")
    rows = list(data.get("top_api_users", []))
    if not rows:
        st.info("No API user connection data available.")
        st.caption(f"Last Updated: {data.get('updated_at', '-')}")
        return

    chart_data = pd.DataFrame(rows).set_index("API User")
