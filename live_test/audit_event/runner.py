...
from lib.database.sync import sync_audit_events
...
class AuditEventFetcher(Protocol):
    def __call__(
        self,
        api: object,
...
def run(
    ...
    enabled_event_ids = {str(sample.get("event_id", ""))}
    metrics = RunMetrics()
    ...
    with (
        patch("lib.database.sync.time_series.sources.audit.report_api.get_audit_events", side_effect=fake_api),
        patch("lib.database.sync.time_series.sources.audit._get_enabled_event_ids", return_value=enabled_event_ids),
